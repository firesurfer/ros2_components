...
{
    std::cout << "addChild called with: " << child->getName() << std::endl;
...
void EntityBase::publishMetaInformation()
{
    ...
    if (set_parameters_results.wait_for(10_s) != std::future_status::ready)
    {
...
void EntityBase::on_child_added(std::shared_ptr<EntityBase> child,std::shared_ptr<EntityBase> parent,int depth)
{
    std::cout << "child"<<child->getName()<< "added to: " <<parent->getName() << " depth: " << depth << std::endl;
    emit childAdded(child,parent, depth+1);
}
